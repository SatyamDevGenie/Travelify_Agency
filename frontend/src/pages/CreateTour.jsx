...
const SelectField = ({ label, value, onChange, options, disabled = false }) => (
  <div>
    ...
    <select
      ...
      onChange={onChange}
      disabled={disabled}
      className={`w-full p-4 border rounded-xl shadow-sm focus:ring-4 focus:ring-indigo-200 focus:border-indigo-500 text-lg ${disabled ? "bg-gray-100 cursor-not-allowed" : ""
        }`}
      required
    >
    ...
    <div className="min-h-screen bg-gray-50 py-16">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <form
          ...
          className="bg-white shadow-2xl rounded-[2.5rem] p-10 space-y-8 border border-gray-100 transition hover:shadow-3xl"
        >
          <div className="grid md:grid-cols-2 gap-8">
            <InputField label="Tour Title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Mystic Himalayan Trek" icon="🏔️" />
            <InputField label="Location / Destination" type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g., Manali, Himachal Pradesh" icon="📍" />
          </div>

          <div className="grid md:grid-cols-2 gap-8">
            <SelectField label="Category" value={category} onChange={(e) => setCategory(e.target.value)} options={categories} />
            <SelectField label="Subcategory / Place" value={subcategory} onChange={(e) => setSubcategory(e.target.value)} options={subcategories} disabled={!category} />
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 pt-4 border-t border-gray-100">
            <InputField label="Price (₹ per person)" type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="e.g., 15000" icon="💰" min="1" />
            <InputField label="Available Slots" type="number" value={availableSlots} onChange={(e) => setAvailableSlots(e.target.value)} placeholder="e.g., 20" icon="👥" min="1" />
          </div>
          ...
          <div className="pt-4 border-t border-gray-100">
            ...
            {image && <p className="text-sm text-green-600 mt-2">✅ File selected: {image.name}</p>}
          </div>
